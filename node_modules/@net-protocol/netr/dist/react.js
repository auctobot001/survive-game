...
function useNetrToken({
  ...
  const { data, isLoading, error } = wagmi.useReadContracts({
    contracts,
    query: { enabled: enabled && !!tokenAddress }
  });
  const tokenMetadata = react.useMemo(() => {
    if (!data || data.length < 9) return void 0;
    if (!data.every((result) => result.status === "success")) return void 0;
    return {
      name: data[0].result,
      symbol: data[1].result,
      deployer: data[2].result,
      image: data[3].result,
      animation: data[4].result,
      fid: data[5].result,
      totalSupply: data[6].result,
      decimals: data[7].result,
      extraStringData: data[8].result
    };
  }, [data]);
  return {
    data: tokenMetadata,
...
function addressToBytes32(address) {
  ...
  if (!viem.isHex(withPrefix)) {
    throw new Error(`Invalid address format: ${address}`);
  }
  return viem.pad(withPrefix, { size: 32 });
}
...
// src/hooks/useNetrPrice.ts
var DEFAULT_REFRESH_INTERVAL = 5e3;
function useNetrPrice({
  chainId,
  tokenAddress,
  enabled = true,
  ...
  const storageKey = react.useMemo(
    () => tokenAddress ? addressToBytes32(tokenAddress) : void 0,
    [tokenAddress]
  );
  const netrContract = react.useMemo(() => {
    try {
  ...
  const { data: storageResult, isLoading: storageLoading } = wagmi.useReadContract({
    address: storage.STORAGE_CONTRACT.address,
    abi: storage.STORAGE_CONTRACT.abi,
    functionName: "get",
    args: storageKey && netrContract ? [storageKey, netrContract.address] : void 0,
    chainId,
    query: { enabled: enabled && !!tokenAddress && !!netrContract }
  });
  const messageIndex = react.useMemo(() => {
    if (!storageResult) return void 0;
    const [, storageValue] = storageResult;
    return decodeBangerStorageData(storageValue)?.messageIndex;
  }, [storageResult]);
  const netContract = react.useMemo(() => {
    try {
      return core.getNetContract(chainId);
    } catch {
      return void 0;
    }
  }, [chainId]);
  const { data: messageResult, isLoading: messageLoading } = wagmi.useReadContract({
    address: netContract?.address,
    abi: netContract?.abi ?? [],
    functionName: "getMessage",
    args: messageIndex !== void 0 ? [messageIndex.toString()] : void 0,
    chainId,
    query: { enabled: enabled && messageIndex !== void 0 && !!netContract }
  });
  react.useEffect(() => {
    if (!messageResult) {
      setPoolAddress(void 0);
      return;
    }
  ...
  react.useEffect(() => {
    ...
    async function fetchPrice() {
      try {
        const rpcUrls = core.getChainRpcUrls({ chainId });
        if (rpcUrls.length === 0) {
        ...
        const slot0 = await actions.readContract(client, {
          address: poolAddress,
...
function useNetrLocker({
  ...
  const lockerData = react.useMemo(() => {
    ...
    return {
      ...
      endTimestamp: data[2].result,
      version: data[3].result
    };
  }, [data]);
  return {
    data: lockerData,
...
exports.useNetrLocker = useNetrLocker;
exports.useNetrPrice = useNetrPrice;
exports.useNetrToken = useNetrToken;
