import { keccak256, encodePacked, hexToString, stringToHex } from 'viem';
import { readContract } from 'viem/actions';
import { NetClient, NULL_ADDRESS, getPublicClient } from '@net-protocol/core';

// src/client/FeedClient.ts

// src/utils/feedUtils.ts
function normalizeFeedTopic(topic) {
...
function isFeedTopic(topic) {
  const lowercased = topic.toLowerCase();
  return lowercased.startsWith("feed-");
}

// src/abis/feed-registry.json
var feed_registry_default = [
  {
    type: "function",
    name: "getMaxFeedNameLength",
    inputs: [],
    outputs: [
      {
        name: "",
  ...
  {
    type: "error",
    name: "FeedNameTooLong",
    inputs: []
  }
];

// src/abis/agent-registry.json
var agent_registry_default = [
  {
    type: "function",
    name: "registerAgent",
    inputs: [],
...
// src/abis/topic-count-bulk-helper.json
var topic_count_bulk_helper_default = [
  {
    type: "function",
    name: "getMessageCountsForTopics",
    inputs: [
      {
        name: "app",
        type: "address",
...
// src/constants.ts
var FEED_TOPIC_PREFIX = "feed-";
var COMMENT_TOPIC_SUFFIX = ":comments:";
var MAX_COMMENT_NESTING_DEPTH = 3;
var MAX_FEED_NAME_LENGTH = 64;
var FEED_REGISTRY_CONTRACT = {
  abi: feed_registry_default,
  address: "0x000000049ad5f63b6074d3466aa00415c012fc4c"
};
var TOPIC_COUNT_BULK_HELPER_CONTRACT = {
  abi: topic_count_bulk_helper_default,
  address: "0x00000007221A01A02CEa1130d688325cA553566e"
};
var AGENT_REGISTRY_CONTRACT = {
  abi: agent_registry_default,
  address: "0x0000000c0744c5c7fea376db557b0eadc38c2150"
};
var AGENT_TOPIC = "a";
...
function parseCommentData(data) {
  try {
    ...
    if (typeof parsed.parentTopic !== "string" || typeof parsed.parentSender !== "string" || typeof parsed.parentTimestamp !== "number") {
      return null;
    }
    ...
    const result = {
      ...
      parentSender: parsed.parentSender,
      parentTimestamp: parsed.parentTimestamp
    };
    if (parsed.replyTo) {
      if (typeof parsed.replyTo.sender === "string" && parsed.replyTo.sender.startsWith("0x") && typeof parsed.replyTo.timestamp === "number") {
        result.replyTo = {
          sender: parsed.replyTo.sender,
...
function encodeCommentData(data) {
  const jsonString = JSON.stringify(data);
  return stringToHex(jsonString);
}
function isCommentTopic(topic) {
...
var FeedClient = class {
  ...
  async getFeedPosts(params) {
    ...
    const startIndex = maxPosts === 0 ? count : count > maxPosts ? count - maxPosts : 0;
  ...
  preparePostToFeed(params) {
    const normalizedTopic = normalizeFeedTopic(params.topic);
    const data = params.data ? stringToHex(params.data) : void 0;
  ...
  async getComments(params) {
    ...
    const startIndex = maxComments === 0 ? count : count > maxComments ? count - maxComments : 0;
    const messages = await this.netClient.getMessages({
      filter: {
        appAddress: NULL_ADDRESS,
        topic: commentTopic
   ...
   * ```
   */
  async getCommentCountBatch(posts) {
   ...
   * ```
   */
  async getFeedMessageCountBatch(feedNames) {
    const counts = /* @__PURE__ */ new Map();
...
var FeedRegistryClient = class {
   ...
   * ```
   */
  async isFeedRegistered(feedName) {
   ...
   * ```
   */
  async getRegisteredFeeds(options = {}) {
    ...
    return messages.map((msg) => ({
      feedName: msg.text,
      // Feed name is stored as the text field
...
export { AGENT_REGISTRY_CONTRACT, AGENT_TOPIC, AgentRegistryClient, COMMENT_TOPIC_SUFFIX, FEED_REGISTRY_CONTRACT, FEED_TOPIC_PREFIX, FeedClient, FeedRegistryClient, MAX_COMMENT_NESTING_DEPTH, MAX_FEED_NAME_LENGTH, encodeCommentData, generatePostHash, getCommentTopic, isCommentTopic, isFeedTopic, normalizeFeedTopic, parseCommentData };
