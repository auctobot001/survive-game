...
// src/hooks/useNetMessages.ts
function useNetMessages(params) {
...
function useNetMessageCount(params) {
  const readContractArgs = useMemo(
...
function useNetMessagesBatchAsync(params) {
  const {
    startIndex = 0,
    endIndex,
    chainId,
  ...
  const batchRanges = useMemo(() => {
    if (endIndex === void 0 || endIndex < startIndex) return [];
  ...
  const configs = useMemo(() => {
    return batchRanges.map(
      ([batchStart, batchEnd]) => getNetMessagesReadConfig({
        filter,
        chainId,
...
export { NetProvider, useNetMessageCount, useNetMessages, useNetMessagesBatchAsync };
